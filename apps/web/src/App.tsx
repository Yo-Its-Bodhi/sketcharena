...
type EcosystemOverview = { accounts: number; wallets: number; rewards: number; activeEntitlements: number; activeCampaigns: number; apps: EcosystemApp[]; audit: EcosystemAudit[] };
...
function Mothership({ token, canOperate, canAdmin }: { token: string; canOperate: boolean; canAdmin: boolean }) {
  ...
  const load = useCallback(async (query = '') => {
    setLoading(true);
    try {
      const [nextOverview, nextAccounts, nextRewards] = await Promise.all([
      ...
      setOverview(nextOverview); setAccounts(nextAccounts); setRewards(nextRewards); setMessage('');
  ...
  return <section className="mothership" aria-label="BodhiX ecosystem control room">
    ...
    {overview ? <>
      <div className="mothership-metrics"><article><b>{overview.accounts}</b><small>ACCOUNTS</small></article><article><b>{overview.wallets}</b><small>VERIFIED WALLETS</small></article><article><b>{overview.rewards}</b><small>REWARDS</small></article><article><b>{overview.activeEntitlements}</b><small>ACTIVE ENTITLEMENTS</small></article><article><b>{overview.activeCampaigns}</b><small>CAMPAIGNS</small></article></div>
      <div className="mothership-apps">{overview.apps.map((app) => <article key={app.id}><i className={app.status}/><b>{app.name}</b><small>{app.status.toUpperCase()}</small></article>)}</div>
