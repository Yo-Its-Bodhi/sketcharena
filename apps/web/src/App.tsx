...
function Arena({ connected, room, prompt, feed, inviteCode, reveal, setReveal, savedRounds, keepRound, reportError, leave }: { connected: boolean; room: RoomView; prompt: string; feed: FeedItem[]; inviteCode: string; reveal: RoundResult | null; setReveal: (value: RoundResult | null) => void; savedRounds: string[]; keepRound: (round: RoundResult) => Promise<ArtworkDocument>; reportError: (message: string) => void; leave: () => void }) {
  ...
  return <motion.main className={`arena arena-screen screen tension-${tension}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
    ...
    <div className="game-grid">
      <aside className={`scoreboard ${mobilePanel === 'players' ? 'mobile-open' : ''}`}><div className="drawer-heading"><h3>THE LINEUP</h3><button onClick={() => setMobilePanel('none')} aria-label="Close players">×</button></div>{room.players.map((player, index) => <motion.div layout className={`score-player ${player.isDrawer ? 'drawing' : ''} ${player.hasGuessed ? 'scored' : ''} ${player.ready ? 'ready' : ''} ${!player.connected ? 'offline' : ''} ${player.id !== me?.id ? 'moderatable' : ''}`} key={player.id}><span className="rank">{index === 0 && player.score > 0 ? '♛' : index + 1}</span><Avatar seed={player.avatarSeed} item={player.avatarItem} frame={player.frameItem}/><div><b>{player.name}</b><small>{player.titleItem ? player.titleItem.replaceAll('-', ' ') : player.isDrawer ? 'DRAWING' : player.hasGuessed ? 'GOT IT' : room.phase === 'lobby' && player.ready ? 'READY' : player.isHost ? 'HOST' : room.phase === 'lobby' ? 'GETTING READY' : 'GUESSING'}</small></div><motion.strong key={`${player.score}-${player.ready}`} initial={{ scale: 1.45, color: '#ffd447' }} animate={{ scale: 1, color: '#f8f5ea' }}>{room.phase === 'lobby' && player.ready ? '✓' : player.score}</motion.strong>{player.id !== me?.id && <button className="player-report" aria-label={`Report ${player.name}`} title={`Report ${player.name}`} onClick={() => setReporting(player)}>!</button>}{me?.isHost && !player.isHost && <button className="player-kick" aria-label={`Remove ${player.name} from the arena`} title={`Remove ${player.name}`} onClick={() => kick(player.id, player.name)}>×</button>}</motion.div>)}</aside>
