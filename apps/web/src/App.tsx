...
import { DRAW_LIMITS, type Ack, type ArtworkDocument, type CanvasRatio, type FeedItem, type LeaderboardPeriod, type LeaderboardResponse, type MatchResult, type MintPreparation, type ModerationReport, type ModerationReportCategory, type PanicArchiveItem, type PlayerProgress, type PlayerView, type RewardEntitlement, type RoomPhase, type RoomSummary, type RoomView, type RoundResult, type SeasonItemDefinition, type Stroke } from '@sketch-arena/protocol';
import { Canvas, type CanvasLayer, type CanvasViewport } from './Canvas';
...
function artworkSvg(title: string, strokes: Stroke[], width = 2400, height = 2400): string {
  ...
  const marks = strokes.map((stroke) => {
    const brush = stroke.brush ?? 'pencil'; const alpha = brush === 'marker' ? .36 : brush === 'airbrush' ? .24 : brush === 'watercolor' ? .28 : brush === 'pastel' ? .62 : brush === 'charcoal' ? .72 : 1;
    const opacity = stroke.tool === 'eraser' ? 1 : (stroke.opacity ?? 1) * alpha;
    ...
    const widthScale = brush === 'technical' ? .45 : brush === 'pencil' ? .7 : brush === 'marker' ? 1.55 : brush === 'airbrush' ? 2.2 : brush === 'watercolor' ? 1.8 : brush === 'calligraphy' ? 1.5 : 1;
    const linecap = ['pixel','marker','calligraphy'].includes(brush) ? 'square' : 'round'; const dash = brush === 'charcoal' ? ` stroke-dasharray="${Math.max(2,stroke.size)},${Math.max(1,stroke.size / 3)}"` : brush === 'pastel' ? ` stroke-dasharray="${Math.max(1,stroke.size / 2)},${Math.max(1,stroke.size / 5)}"` : '';
    const filter = brush === 'neon' ? ' filter="url(#neon-glow)"' : brush === 'airbrush' || brush === 'watercolor' ? ' filter="url(#soft-brush)"' : '';
    const style = `fill="none" stroke="${color}" stroke-opacity="${opacity}" stroke-width="${stroke.size * 2 * widthScale}" stroke-linecap="${linecap}" stroke-linejoin="round"${dash}${filter}`;
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><title>${safeTitle}</title><defs><filter id="soft-brush"><feGaussianBlur stdDeviation="5"/></filter><filter id="neon-glow" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="7" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs><rect width="${width}" height="${height}" fill="#f4f0e8"/>${marks}</svg>`;
}
