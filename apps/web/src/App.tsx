...
export function App() {
  ...
  return <MotionConfig reducedMotion="user"><div className="app">
    ...
    {screen !== 'arena' && screen !== 'studio' && <WorldSignature/>}
  </div></MotionConfig>;
}
...
function SeasonBook({ progression, catalog, equipItem, close }: { progression: PlayerProgress; catalog: SeasonItemDefinition[]; equipItem: (itemId: string) => Promise<void>; close: () => void }) {
  ...
  const xpIntoLevel = progression.xp % 1_000;
  const achievementNames: Record<string, string> = { 'first-mess': 'Entered the First Mess', 'crowned-chaos': 'Crowned Chaos', 'panic-button': 'Panic Button', 'certified-mess': 'Certified Mess', 'first-mess-finisher': 'First Mess Finisher' };
  const lane = <section className="pass-lane free-lane"><header><span>✦</span><div><small>FREE BETA TRACK</small><b>THE FIRST MESS</b><p>Real rewards for helping us prove the game works.</p></div><em>ACTIVE</em></header><div className="season-track">{SEASON_FREE_TRACK.map((tier) => { const earned = progression.rewards.some((reward) => reward.campaignId === tier.campaignId); return <article className={earned ? 'earned' : progression.level >= tier.level ? 'incoming' : 'locked'} key={`free-${tier.level}`}><span>{tier.icon}</span><small>LEVEL {tier.level}</small><b>{tier.name}</b><p>{tier.detail}</p><em>{earned ? '✓ EARNED' : progression.level >= tier.level ? 'DELIVERING' : 'LOCKED'}</em></article>; })}</div></section>;
...
function WorldSignature() {
  return <footer className="world-signature">
    <span>WELCOME TO ANOTHER WEIRD WORLD BY BODHIX.</span>
    <a href="https://bodhix.io/" target="_blank" rel="noreferrer">BODHIX.IO ↗</a>
    <small>© {new Date().getFullYear()} BODHIX</small>
  </footer>;
}
