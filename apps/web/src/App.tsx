...
type EcosystemClaim = { id:string;app_id:string;quantity:number;status:'reserved'|'fulfilled'|'rejected'|'reversed';reserved_at:string;account_name:string;reward_code:string;reward_name:string;external_reference?:string };
...
function Mothership({ token, canOperate, canAdmin }: { token: string; canOperate: boolean; canAdmin: boolean }) {
  ...
  const load = useCallback(async (query = '') => {
    ...
    try {
      const [nextOverview, nextAccounts, nextRewards, nextCampaigns, nextClaims] = await Promise.all([
        ...
        request<EcosystemClaim[]>('/api/admin/ecosystem/claims?status=reserved'),
      ]);
  ...
  const saveCampaign = async () => {
    ...
    try {
      const saved = await request<EcosystemCampaign>('/api/admin/ecosystem/campaigns', { method:'POST', body:JSON.stringify({ code:campaignCode,name:campaignName,rewardCode,appId:campaignAudience==='app'?campaignApp:undefined,audience,quantityPerAccount:grantQuantity,hidden:true }) });
  ...
  const resolveClaim = async (claim:EcosystemClaim,status:'fulfilled'|'rejected') => {
    try { await request(`/api/admin/ecosystem/claims/${claim.id}`,{method:'PATCH',body:JSON.stringify({status,confirmation:`MARK CLAIM ${status.toUpperCase()}`})}); setMessage(status==='fulfilled'?'CLAIM FULFILLED · Receipt preserved.':'CLAIM REJECTED · Reserved quantity restored.'); await load(search); }
    catch(error){ setMessage(error instanceof Error?error.message:'Claim could not be resolved'); }
  };
  const toggleAccount = (id: string) => { setSelected((current) => current.includes(id) ? current.filter((value) => value !== id) : [...current, id]); setGrantPreview(null); };
