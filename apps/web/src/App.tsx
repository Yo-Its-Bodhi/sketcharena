...
type Screen = 'landing' | 'lobby' | 'arena' | 'studio' | 'vault' | 'archive' | 'leaderboard' | 'afterparty' | 'backstage';
...
export function App() {
  const [screen, setScreen] = useState<Screen>(() => location.pathname === '/backstage' ? 'backstage' : location.pathname === '/archive' ? 'archive' : location.pathname === '/leaderboard' ? 'leaderboard' : 'landing');
  ...
  return <MotionConfig reducedMotion="user"><div className="app">
    ...
    <AnimatePresence mode="wait">
      ...
      {screen === 'lobby' && <Lobby key="lobby" name={name} rooms={rooms} progression={progression} acknowledgeReward={acknowledgeReward} equipItem={equipItem} redeemPromotion={redeemPromotion} create={createRoom} join={(id) => joinRoom({ roomId: id })} joinCode={(code) => joinRoom({ inviteCode: code })} studio={() => setScreen('studio')} vault={() => setScreen('vault')} archive={openArchive} leaderboard={openLeaderboard}/>}
...
function Lobby({ name, rooms, progression, acknowledgeReward, equipItem, redeemPromotion, create, join, joinCode, studio, vault, archive, leaderboard }: { name: string; rooms: RoomSummary[]; progression: PlayerProgress | null; acknowledgeReward: (rewardId: string) => void; equipItem: (itemId: string) => Promise<void>; redeemPromotion: (code: string) => Promise<string>; create: (options: CreateRoomOptions) => void; join: (id: string) => void; joinCode: (code: string) => void; studio: () => void; vault: () => void; archive: () => void; leaderboard: () => void }) {
  ...
  return <motion.main className="screen lobby lobby-v2" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
    <header className="topbar brand-topbar"><Brand/><MainNav active="play" play={() => undefined} studio={studio} vault={vault} archive={archive} leaderboard={leaderboard}/><div className="player-chip"><Avatar seed={name.length * 91} item={progression?.equipped.avatar} frame={progression?.equipped.frame}/><span><small>{progression?.equipped.title?.replaceAll('-', ' ') ?? 'TONIGHT’S TROUBLEMAKER'}</small><b>{name}</b></span></div></header>
...
const SEASON_PREMIUM_TRACK = [
  { level: 1, icon: '●', name: 'GREEN CHAOS', detail: 'Premium player avatar', campaignId: 'season-0-premium-level-1' },
  { level: 2, icon: '▰', name: 'RIOT MARKER', detail: 'Wet stacking marker brush', campaignId: 'season-0-premium-level-2' },
  { level: 3, icon: '✦', name: 'BONUS ARCHIVE', detail: '1 additional free mint', campaignId: 'season-0-premium-level-3' },
  { level: 5, icon: 'ϟ', name: 'NEON PANIC', detail: 'Electric glow brush', campaignId: 'season-0-premium-level-5' },
  { level: 7, icon: '🧨', name: 'TINY FIRE', detail: 'Premium live reaction', campaignId: 'season-0-premium-level-7' },
  { level: 10, icon: '✦✦', name: 'DOUBLE ARCHIVE', detail: '2 additional free mints', campaignId: 'season-0-premium-level-10' },
  { level: 12, icon: '▧', name: 'CHAOS CHARCOAL', detail: 'Dry fibre + dust brush', campaignId: 'season-0-premium-level-12' },
  { level: 15, icon: '▣', name: 'VELVET ROPE', detail: 'Legendary profile frame', campaignId: 'season-0-premium-level-15' },
  { level: 18, icon: 'Aa', name: 'PRO DISASTER', detail: 'Professional Disaster title', campaignId: 'season-0-premium-level-18' },
  { level: 20, icon: '♛', name: 'GOLDEN CHAOS', detail: 'Legendary season avatar', campaignId: 'season-0-premium-level-20' },
];
...
function SeasonBook({ progression, catalog, equipItem, close }: { progression: PlayerProgress; catalog: SeasonItemDefinition[]; equipItem: (itemId: string) => Promise<void>; close: () => void }) {
  ...
  const lane = (name: string, detail: string, premium: boolean, track: typeof SEASON_FREE_TRACK) => <section className={`pass-lane ${premium ? 'premium-lane' : 'free-lane'}`}><header><span>{premium ? '♛' : '✦'}</span><div><small>{premium ? 'PREMIUM TRACK' : 'FREE TRACK'}</small><b>{name}</b><p>{detail}</p></div><em>{premium ? progression.battlePass === 'premium' ? 'ACTIVE' : 'LOCKED' : 'ACTIVE'}</em></header><div className="season-track">{track.map((tier) => { const earned = progression.rewards.some((reward) => reward.campaignId === tier.campaignId); const accessible = !premium || progression.battlePass === 'premium'; return <article className={earned ? 'earned' : accessible && progression.level >= tier.level ? 'incoming' : 'locked'} key={`${premium ? 'premium' : 'free'}-${tier.level}`}><span>{tier.icon}</span><small>LEVEL {tier.level}</small><b>{tier.name}</b><p>{tier.detail}</p><em>{earned ? '✓ EARNED' : !accessible ? 'PREMIUM' : progression.level >= tier.level ? 'DELIVERING' : 'LOCKED'}</em></article>; })}</div></section>;
  return <motion.div className="loot-overlay season-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => event.target === event.currentTarget && close()}><motion.section ref={dialogRef} tabIndex={-1} className="season-book" role="dialog" aria-modal="true" aria-labelledby="season-title" initial={{ y: 45, scale: .94, rotate: 1 }} animate={{ y: 0, scale: 1, rotate: 0 }} exit={{ y: 30, opacity: 0 }}><button className="setup-close" onClick={close} aria-label="Close Season progress">×</button><header><div className="loot-weirdos" aria-hidden="true"><ChaosFace variant={0}/><ChaosFace variant={1}/><ChaosFace variant={2}/></div><small>SEASON 0 BETA · THE FIRST MESS</small><h2 id="season-title">YOUR PANIC<br/>PROGRESS.</h2><p>One XP bar advances both tracks. Free rewards are for everybody; Premium adds better cosmetics and extra Mint Credits, never score or gameplay power. Everything earned stays yours.</p></header><div className="season-level-card"><span><small>CURRENT LEVEL</small><b>{progression.level}</b></span><div><strong>{progression.battlePass === 'premium' ? '♛ BOTH TRACKS ACTIVE' : '✦ FREE TRACK ACTIVE'}</strong><i><b style={{ width: `${xpIntoLevel / 10}%` }}/></i><em>{xpIntoLevel} / 1000 XP TO LEVEL {progression.level + 1}</em></div></div>{lane('FREE PANIC PASS', 'Meaningful rewards for every player.', false, SEASON_FREE_TRACK)}{lane('PREMIUM PANIC PASS', 'A second lane of genuinely usable cosmetics and bonus archive credits.', true, SEASON_PREMIUM_TRACK)}<div className="pass-fairness"><b>NO PAY-TO-WIN. EVER.</b><span>Premium does not change Chaos Score, guessing points, matchmaking, drawing time or leaderboard rank.</span></div><div className="collection-ledger"><section><small>ACHIEVEMENTS & BADGES</small><h3>{progression.achievements.length} COLLECTED</h3>{progression.achievements.length ? <div>{progression.achievements.map((id) => <span key={id}>★ {achievementNames[id] ?? id.replaceAll('-', ' ')}</span>)}</div> : <p>Your trophy shelf is suspiciously tidy.</p>}</section><section><small>USABLE DROPS</small><h3>{progression.items.length} COLLECTED</h3>{progression.items.length ? <div className="cosmetic-grid">{progression.items.map((id) => { const item = catalog.find((entry) => entry.id === id); const equipped = item ? progression.equipped[item.slot] === id : false; return <button className={equipped ? 'equipped' : ''} onClick={() => void equipItem(id)} key={id}><i style={{ background: item?.previewColor }}>{item?.glyph}</i><span><b>{item?.name ?? id.replaceAll('-', ' ')}</b><small>{item?.slot ?? 'cosmetic'} · {item?.rarity ?? 'earned'}</small></span><em>{equipped ? '✓ EQUIPPED' : 'EQUIP'}</em></button>; })}</div> : <p>Level up to make this pile weirder.</p>}</section></div></motion.section></motion.div>;
}
