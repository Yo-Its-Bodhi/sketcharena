...
type ContractAccessChoice = 'approve' | 'remove-approval' | 'block' | 'unblock' | 'require-allowlist' | 'open-vouchers';
...
interface CreateRoomOptions { name: string; category: 'chaos' | 'classic' | 'crypto'; isPrivate: boolean; maxPlayers: number; roundSeconds: 30 | 45 | 60; }
...
export function App() {
  ...
  return <MotionConfig reducedMotion="user"><div className="app">
    ...
    <AnimatePresence mode="wait">
      ...
      {screen === 'arena' && room && <Arena key="arena" connected={connected} room={room} prompt={prompt} feed={feed} inviteCode={inviteCode} reveal={reveal} setReveal={setReveal} reportError={setError} leave={() => socket.emit('room:leave', (ack) => { handleAck(ack); if (!ack.ok) return; setRoom(null); setFeed([]); setInviteCode(''); setScreen('lobby'); socket.emit('rooms:subscribe'); })}/>}
  ...
  return <motion.main className="screen lobby lobby-v2" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
    ...
    <AnimatePresence>{setupOpen && <motion.div className="setup-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => event.target === event.currentTarget && setSetupOpen(false)}>
      <motion.section ref={setupDialogRef} tabIndex={-1} className="setup-sheet" role="dialog" aria-modal="true" aria-labelledby="room-setup-title" initial={{ y: 40, rotate: 1, opacity: 0 }} animate={{ y: 0, rotate: 0, opacity: 1 }} exit={{ y: 30, opacity: 0 }}>
        ...
        <fieldset><legend>PROMPT DECK</legend>{(['chaos','classic','crypto'] as const).map((value) => <button type="button" className={category === value ? 'selected' : ''} onClick={() => setCategory(value)} key={value}><b>{value === 'chaos' ? '?!' : value === 'classic' ? '✎' : '₿'}</b><span>{value}<small>{value === 'chaos' ? 'Unhinged & weird' : value === 'classic' ? 'Pictionary energy' : 'Web3 nonsense'}</small></span></button>)}</fieldset>
        <div className="setup-rules"><RulePicker label="PLAYERS / DRAWINGS" values={[4,6,8]} value={maxPlayers} setValue={setMaxPlayers}/><div className="fixed-match-rule"><small>MATCH LENGTH</small><b>{maxPlayers} drawings total</b><span>Turns rotate fairly through everyone who joins.</span></div><RulePicker label="SECONDS" values={[30,45,60]} value={roundSeconds} setValue={(value) => setRoundSeconds(value as 30 | 45 | 60)}/></div>
...
function Backstage({ home }: { home: () => void }) {
  ...
  const submitContractAccess = async () => {
    ...
    try {
      const body = accessChoice === 'require-allowlist' || accessChoice === 'open-vouchers' ? { action: 'set-allowlist', enabled: accessChoice === 'require-allowlist' }
...
function Arena({ connected, room, prompt, feed, inviteCode, reveal, setReveal, reportError, leave }: { connected: boolean; room: RoomView; prompt: string; feed: FeedItem[]; inviteCode: string; reveal: RoundResult | null; setReveal: (value: RoundResult | null) => void; reportError: (message: string) => void; leave: () => void }) {
  ...
  return <motion.main className={`arena arena-screen screen tension-${tension}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
    ...
    <AnimatePresence>{reveal && <Reveal result={reveal} mine={reveal.drawerId === me?.id} reportError={reportError} close={() => setReveal(null)}/>}</AnimatePresence>
...
function Reveal({ result, mine, reportError, close }: { result: RoundResult; mine: boolean; reportError: (message: string) => void; close: () => void }) {
...
type MintStatusResponse = { enabled: boolean; contractControlsEnabled: boolean; collection: string; season: string; chainId?: number; chainName: string; nativeCurrency: { name: string; symbol: string; decimals: number }; blockExplorerUrl?: string; standardPriceWei?: string; firstMintFree: boolean };
...
function MintMoment({ item, close, confirmed }: { item: ArtworkDocument; close: () => void; confirmed: (mint: MintPreparation) => void }) {
  const dialogRef = useDialogFocus<HTMLDivElement>(true, close); const [service, setService] = useState<MintStatusResponse | null>(null); const [phase, setPhase] = useState<'checking' | 'ready' | 'connecting' | 'prepared' | 'sending' | 'pending' | 'failed' | 'success' | 'unavailable'>('checking');
