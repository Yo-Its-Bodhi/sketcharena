...
async function waitForWalletReceipt(provider: EthereumProvider, hash: string): Promise<void> {
  for (let attempt = 0; attempt < 80; attempt += 1) {
    const receipt = await provider.request({ method: 'eth_getTransactionReceipt', params: [hash] }) as { status?: string } | null;
    if (receipt) { if (receipt.status && BigInt(receipt.status) === 0n) throw new Error('WSHIDO approval was rejected by the network. No mint fee was charged.'); return; }
    await new Promise((resolve) => window.setTimeout(resolve, 1_500));
  }
  throw new Error('WSHIDO approval is still pending. Wait for it to confirm, then try minting again.');
}
