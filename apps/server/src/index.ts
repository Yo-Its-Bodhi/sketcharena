...
async function awardMatchProgress(result: MatchResult): Promise<void> {
  ...
  for (const player of result.standings) {
    ...
    if (result.winner?.sessionId === player.sessionId) await progression.grant({ sessionIds: [player.sessionId], kind: 'achievement', amount: 1, itemId: 'crowned-chaos', reason: 'Won a Sketch Arena match', campaignId: 'achievement-crowned-chaos', idempotencyKey: `${matchKey}-winner`, actor: 'system:match' });
