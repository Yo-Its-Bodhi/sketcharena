...
describe('BodhiX server app adapter', () => {
  it('keeps the app session in the authorization header and binds the app ID', async () => {
    const request = vi.fn(async (url: string | URL | Request, init?: RequestInit) => { void url; void init; return new Response(JSON.stringify({ account: { id: 'one', name: 'Bodhi', secured: true }, appId: 'carnival', expiresAt: 1, entitlements: [], xp: [], memberships: [] }), { status: 200, headers: { 'content-type': 'application/json' } }); });
  ...
  it('exchanges a PKCE code without placing the resulting app token in a URL', async () => {
    const request = vi.fn(async (url: string | URL | Request, init?: RequestInit) => { void url; void init; return new Response(JSON.stringify({ account: { id: 'one', name: 'Bodhi', secured: true }, appSession: { token, expiresAt: 1 } }), { status: 200, headers: { 'content-type': 'application/json' } }); });
